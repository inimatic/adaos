...
not only a data-mode label. The initial adapters cover owned native
request/response skills with explicitly declared SQLite stores and bounded
content-addressed blob stores. Service workers, shared mutable stores and
credential files need their own adapters.
...
Durable uploaded or generated bytes declare `storage.blob`. Core snapshots the
Application-owned content-addressed blob root alongside relational data,
validates bounded file count/size, relative paths and hashes, rejects links and
path escape, and adopts the reviewed Beta root on Stable acceptance. References
stored in relational rows remain valid because blob identity is content based.
Removing `storage.blob` while Stable owns bytes requires an explicit migration
adapter; package assets, temporary uploads and arbitrary filesystem paths are
never inferred as Application data.
