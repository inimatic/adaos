...
| 6. Activation | Partial: immutable ProjectRelease, accepted trial, stable promotion, WorkspaceLock activation, permission/migration gates, health verification, and rollback are validated locally; clean-stand acceptance and setup-plan completion remain open. | Partial: durable operation and one-shot reconciliation exist; delayed observation and rollback UX remain open. | Optional setup-assistant UX. | Automatic setup authoring/execution is postponed until Publication owns its contract. |
