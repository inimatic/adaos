...
- [Application Lifecycle, Distribution, and Feedback](application-lifecycle-and-distribution.md): canonical Application identity, DEV/Trial/prerelease/stable lifecycle, Applications product surface, publisher subnet authority, SDK/MCP boundary, installation/reference accounting, and encrypted Development Report relay
- [Managed Applications and Projects](managed-applications-and-projects.md): owner/Project relationship, lifecycle invariants, default inventory filtering, inverse detail projections, and Home pin semantics for Applications created by a workbench
- [Application Lifecycle and Distribution Roadmap](application-lifecycle-and-distribution-roadmap.md): prioritized sequence from Application Core and SDK/MCP through Builder dogfooding, trusted prerelease publication, Development Reports, end-to-end release proof, and deferred Root Guard hardening
- [Application Access, Permissions, and Roles](application-access-permissions.md): target bridge between Applications and Personalization for Application permission profiles, Application-defined roles, per-user/child/guest access grants, secrets, conversational approvals, Users & Access, permission observability, and Builder final verification
- [Application Access, Permissions, and Roles Roadmap](application-access-permissions-roadmap.md): MoSCoW sequence for landing the first enforceable Application permission/role/access/final-verification slice without turning the MVP into a general IAM platform
- [Application Registry Projection](application-registry-projection.md): target private SQLite read model and roadmap for fast Application catalog/search, startup snapshots, background validation, permission declaration indexes, federated Application facts, and browser-safe projection export
- [Capability, Binding, and State Separation](capability-binding-state-separation.md): normative separation of semantic Application requirements, capability/state contracts, portable implementations, immutable delivery packages, local bindings, state spaces, resolution, evidence, and activation planning
- [Capability, Binding, and State Separation Roadmap](capability-binding-state-separation-roadmap.md): contract, resolver, state identity, planning, CRUD/domain proof, freshness, evolution, and benchmark implementation sequence
- [Semantic Application Composition And Evolution Roadmap](application-semantic-composition-roadmap.md): linear integration path from user intent and executable Prototype through semantic resolution, package materialization, Trial/Stable evolution, reusable-package extraction, Applications observability, and legacy retirement
- [Project Composition, Presentation, and Development Context](project-composition-and-development-context.md): current `Project*` compatibility model for composition, presentation/launch targets, local model-facing artifact groups, registry classification, and policy-scoped Builder Development Sessions
...
- [Declarative Resource Workbench](declarative-resource-workbench.md): reusable target model for typed resources, queries, operations, workflow-bound actions, i18n/naming, role/capability access, evidence, events, observability, Builder inspection, and declarative CRUD-style surfaces
- [Declarative Resource Workbench Roadmap](declarative-resource-workbench-roadmap.md): prioritized delivery plan for Dev Ticket resource proof, Demo Metrics harness, Builder observability, Notebook migration, Media Center selective adoption, Research Workbench adoption, channel routing, i18n, access, privacy, and readiness hardening
...
- [Subnet Knowledge Fabric Target Architecture](subnet-knowledge-fabric.md): canonical cross-domain resource knowledge, source observations, reusable artifacts, optional versions, aliases, i18n, hybrid/latent retrieval, and actionable UI/dialog/voice boundary
- [Subnet Knowledge Fabric Roadmap](subnet-knowledge-fabric-roadmap.md): prioritized contracts, implementation, product adoption, and production evidence gates for the shared Knowledge Fabric
...
- [Research Compilation and Autonomous-Science Evaluation Program](research-compilation-and-sota-program.md): scientific-problem-to-engineering bridge, stage-dependent typing, context visibility, TLP C0-C4 calibration, comparative metrics, and evidence bar for a defensible SOTA claim
...
- [Browser Media Runtime](browser-media-runtime.md): one browser-session owner for microphone/camera leases, voice controllers, TTS/audio output, media activity, and the compact communication-channel informer across stationary and Android-hosted browser surfaces
...
- [Device Access and Browsers](device-access-and-browsers.md): target architecture for durable device identity, browser and member access policy, the Devices area inside an application-centric desktop, and reusable access management surfaces
