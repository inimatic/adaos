...
- HTTP diagnostics must not become an unbounded second source of truth for rendered desktop state. A narrow recovery exception is allowed when a command acknowledgement names the expected scenario and live Yjs is semantically inconsistent: the client may use an exact-scenario materialization snapshot as a render overlay with a short TTL. A structurally `ready` live document from another scenario does not supersede that overlay; only convergence to the expected `ui.current_scenario` does. The server must first repair and rebroadcast the selector/projection commit. If that full-state broadcast still does not converge locally, the client may causally commit only the runtime-owned projection branches from the same exact-scenario snapshot; operational branches remain live-only and untouched. This recovery path must not report state-sync healthy until the live document itself matches the expected scenario.
