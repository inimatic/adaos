...
Команда выводит навыки, найденные в каталоге `workspace/dev/{node_id}/skills` текущего узла.
В табличном представлении отображаются четыре столбца:
...
- `Path` — путь к каталогу навыка в рабочем пространстве.
...
Перечисляет сценарии из `workspace/dev/{node_id}/scenarios` с теми же полями `name`, `version`, `updated_at` и `path`.
