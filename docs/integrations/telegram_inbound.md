...
## Диалоговый маршрут текста

Текстовый `ChatInputEvent` не вызывает NLU или навык напрямую. Hub приводит как
NATS envelope, так и raw HTTP fallback к одному событию
`dialog.user_message`. Далее Router:

1. восстанавливает активный диалог или распознаёт явно указанного агента,
   например `Строитель, ...`;
2. передаёт текст владельцу диалога либо в общий NLU fallback;
3. проецирует ответ `io.out.chat.append` обратно в
   `tg.output.<bot_id>.chat.<chat_id>`.

Поля `bot_id`, `hub_id`, `chat_id`, `user_id`, `update_id`, `reply_to`, язык и
trace сохраняются только как транспортный контекст. Идентичность проекта или
Change определяется каноническим topic/thread Builder, а не Telegram chat id.

Перед публикацией `dialog.user_message` Hub атомарно фиксирует
`idempotency_key` входящего update в локальном conversation store. Повтор того
же update не исполняется, а использование того же ключа с другим содержимым
считается конфликтом. После неопределённого сбоя изменяющая состояние команда
не повторяется автоматически: пользователь отправляет новое сообщение.
