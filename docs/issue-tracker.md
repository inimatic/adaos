...
| `RT-POST-INCIDENT-001` / `LRLT-008` | in progress | Checkpoints `.797-.799` on `.30` proved strict single-owner NATS cutover gaps of 1.445-2.954 s and kept HTTP/heartbeat responsive under a forced SQLite exclusive lock. `.799` removed Root registration from the request/executor critical path: two observed Root registrations returned 200 in at most 35 ms while durable persistence retried a separate 5.017 s database-lock failure. The same run exposed a protocol defect rather than a network or skill failure: the old sidecar replayed 32 already-sent route frames before a new `/ws/subnet` hello, producing thousands of `hello_required`/1002 reconnect cycles. Core `.800` removes arbitrary replay, requires a fresh `hello/hello.ack` for session-aware resume, fails protocol-opaque tunnels closed with 1012, publishes exact route-session counters, and isolates the thin reliability beacon from the default executor. Deploy `.800`, run a correlated route-socket fault plus database/executor pressure and a six-minute soak, then repeat the aligned node/Root log analysis. Preserve the run id, clock offset, node/sidecar/Root HTTP/nginx/NATS/browser logs, route-session counters, skill-handler/watchdog/circuit evidence, and process CPU/RSS/disk/network-I/O lookback. Explicitly evaluate skill activity, subprocesses, media indexing, and large downloads as contributors; do not close from static or immediate fault proof alone. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md) |
| `RT-POST-INCIDENT-REVIEW-002` | open | After the `.800` fault-and-soak evidence is captured, perform a separate later review of the preserved logs and counter deltas. Re-check for hidden reconnect loops, competing route owners, stale readiness, event-loop or executor starvation, persistence backlogs, skill circuit activity, downloads/media work, and contradictions between browser, node, sidecar, and Root state. This review remains open even when the first `.800` campaign passes. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md) |
