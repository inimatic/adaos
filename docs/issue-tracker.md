...
| ID | State | Work and next proof | Owner |
| --- | --- | --- | --- |
| `RT-FANOUT` / `SKILL-CHANNEL-001` / `LRLT-001` / `LRLT-002` / `HMG-001` / `HMG-002` / `HMG-006` | ready for stand | Synchronous skill subscriptions now run in a dedicated bounded executor with per-handler admission, watchdog incidents, pre-incident process/I/O evidence, and strict validation against obvious blocking calls in async subscriptions. An independent thread now captures the live event-loop stack and skill attribution even while asyncio itself is blocked. Operation events are latest-state bounded, and Infra State no longer rebuilds its full git inventory for an operation row patch. On `.30`, prove a synthetic blocking subscription cannot delay HTTP, `/ws`, `/yws`, or hub-root heartbeats; prove overload remains attributed and visible rather than silently dropped. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md), [Runtime Guarding](architecture/runtime-guarding.md), [Rebuild Lag Hardening](architecture/realtime-rebuild-lag-hardening.md) |
...
| `RT-POST-INCIDENT-001` / `LRLT-008` | ready for stand | After deploying the channel and skill-execution hardening, run a correlated node/Root fault and soak campaign, then repeat the log analysis. Preserve a run id and aligned node, sidecar, Root HTTP/nginx/NATS, browser YWS attempts, incident-registry, clock-offset, skill-handler, and process CPU/RSS/disk/network-I/O evidence for at least six minutes before and after the fault. Explicitly evaluate skill activity, subprocesses, and large downloads as possible contributors; verify routed YWS has one retry owner, direct WSS stays healthy, sidecar projects `standby`, and controlled failback creates no competing remote sessions. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md) |
