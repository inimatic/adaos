...
| ID | State | Work and next proof | Owner |
| --- | --- | --- | --- |
| `RT-FANOUT` / `LRLT-001` / `LRLT-002` / `HMG-001` / `HMG-002` / `HMG-006` | in progress | Single-flight cached supervisor projection, latest-state bounded core-update fanout, and Teacher startup I/O are implemented with regression coverage. The first `.30` post-cutover window had no errors, recovering transitions, or NATS disconnects and one 287.7 ms lag warning; keep event-loop-affine handlers on their owner loop and run the longer pressure evidence before closure. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md), [Rebuild Lag Hardening](architecture/realtime-rebuild-lag-hardening.md) |
