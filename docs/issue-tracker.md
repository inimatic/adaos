...
| --- | --- | --- | --- |
| `TEST-001` / `MRI-003` | in progress | The complete process-isolated baseline now collects `2871` tests: `2855` passed, `8` failed, `8` skipped, and `0` errored on 2026-07-23. Close the remaining API module-state leak, Neural installer staging failure, and six NLU Teacher validation/apply failures without reverting newer scenario, routing, Builder, or NLU contracts; retain JUnit evidence for every segment. | [MVP Release Evidence](architecture/mvp-release-evidence.md) |
