...
| `RT-POST-INCIDENT-001` / `LRLT-008` | in progress | `.803` on `.30` proved automatic sidecar generation upgrade and current-liveness settle: the planned sidecar restart recovered NATS once in 2.365 s under about 90% I/O PSI with no redundant forced reconnect. `.804` then completed A/B promotion under `full avg10=73.49%` I/O pressure; sidecar readiness passed 48/48 samples and the active runtime had only three short cutover refusals. A controlled duplicate member identity proved terminal handling: the displaced downstream received `4001 link_replaced`, the survivor remained usable, `upstream_terminal_close_total` increased once, and `session_resume_total` did not move. The first exact NATS loopback fault kept runtime HTTP at 124/124 but failed the no-flapping criterion: with PID discovery disabled by loaded `y_py`, synchronous listener snapshots still connected to the NATS data port; after the real client was cut, five observation sockets each opened and immediately closed a remote WSS session before the NATS supervisor recovered. Checkpoint `.805` moves this remaining snapshot path to control-port readiness and restores the lockfile version so `uv sync --locked` does not spend about 41 seconds failing before pip fallback. Earlier residual fixes also keep candidates passive before promotion and move supervisor-channel config/SQLite collection off the event loop. No watchdog stall named a skill (`skill_candidates=[]`); deployed `infrastate_skill 0.75.66` offloads persistence and bounds subscription concurrency, although one offloaded completion took 0.464 s during pressure. Windows recorded neither BITS activity nor adapter/network warnings around the original 2026-08-13 incident. Original Root application logs and the matching repo-local state window are not available as bounded retrospective evidence, so attribution remains limited rather than absolute. Deploy `.805`, prove observation no longer increments `local_client_total`, repeat the exact route/NATS faults, and complete a real browser plus node/sidecar/Root soak of at least six minutes before closure. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md) |
