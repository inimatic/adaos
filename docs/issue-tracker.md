...
| ID | State | Work and next proof | Owner |
| --- | --- | --- | --- |
| `RT-FANOUT` / `LRLT-001` / `LRLT-002` / `HMG-001` / `HMG-002` / `HMG-006` | in progress | Single-flight cached supervisor projection, latest-state bounded core-update fanout, off-loop synchronous subscribers, and Teacher startup I/O are implemented with regression coverage. Run target-stand pressure evidence and continue the per-skill slow-handler audit before closure. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md), [Rebuild Lag Hardening](architecture/realtime-rebuild-lag-hardening.md) |
