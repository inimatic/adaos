...
| `BUILDER-CHANNEL-ACCEPTANCE` | ready for stand | Complete one human wide/compact Web pass and one live Telegram pairing: addressed “что выбрано?”, canonical project topic, the same bounded contextual controls, one callback response, and delivery receipt. Automated projection/token/callback tests pass locally; mutating controls without a registered executor are intentionally withheld and hard channel parity remains out of scope. | [Builder Roadmap](architecture/builder-roadmap.md#phase-11-conversational-development-control-plane), [Builder Conversational Development](architecture/builder-conversational-development.md#channel-capability-boundary) |
