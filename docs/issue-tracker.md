...
| ID | State | Work and next proof | Owner |
| --- | --- | --- | --- |
| `RT-FANOUT` / `LRLT-001` / `LRLT-002` / `HMG-001` / `HMG-002` / `HMG-006` | in progress | Finish single-flight snapshot handling, bounded rebuild fanout, cached/off-loop heavy reads, and per-skill regression coverage. Recent materialization work closes several amplifiers, but stand pressure evidence is still required. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md), [Rebuild Lag Hardening](architecture/realtime-rebuild-lag-hardening.md) |
| `RUNTIME-DECOMPOSITION` / `WSR-001` | active | Ownership transfer is implemented locally: webspace task/cache/recovery/projection/materialization owners, Bootstrap lifecycle/NATS-route/root/status owners, and Supervisor process/update/recovery/memory plus thin FastAPI adapter are covered by component and compatibility regressions. Preserve pointer-first switching, sidecar transport authority, no-op branch identity, and public contracts; close after the bounded `.30` delivery checkpoint. Physical compatibility-file slimming remains follow-up cleanup without alternate state ownership. | [Webspace Pointer/Projection](architecture/webspace-scenario-pointer-projection-roadmap.md#7-runtime-ownership-decomposition), [Webspace Evolution](architecture/webspace-evolution-roadmap.md#runtime-decomposition-required-by-these-gaps), [Realtime Reliability](architecture/realtime-reliability-roadmap.md#bootstrap-decomposition-constraint), [AdaOS Supervisor](architecture/adaos-supervisor.md#implementation-decomposition-boundary) |
