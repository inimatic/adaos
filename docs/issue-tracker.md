...
| `RT-POST-INCIDENT-001` / `LRLT-008` | in progress | `.803` on `.30` proved automatic sidecar generation upgrade and current-liveness settle: the planned sidecar restart recovered NATS once in 2.365 s under about 90% I/O PSI with no redundant forced reconnect. A later exact loopback socket fault restored current root diagnostics in 0.717 s and sidecar `remote_ready` in 2.251 s while all 565/565 runtime pings succeeded. Root `backend-b`, NATS, and ingress remained healthy with zero container restarts and showed matching graceful close/new-subscription boundaries. Residual causes found by this pass are addressed in the next checkpoint: sidecar incorrectly resumed terminal `4001 link_replaced` and sustained competing `/ws/subnet` identities; the generic FastAPI subnet hook could start a member candidate before promotion; and `/api/node/reliability/supervisor-channel` performed `load_config -> SQLite ensure_schema` on the event loop, with the watchdog measuring 956.673 ms during disk pressure. The 102 s ext4 venv copy already used `ionice -c3` but still produced high PSI. No watchdog stall named a skill (`skill_candidates=[]`), and Windows recorded neither BITS activity nor adapter/network warnings around the original 2026-08-13 incident. Original Root application logs and the matching repo-local state window are not available as bounded retrospective evidence, so attribution remains limited rather than absolute. Deploy this checkpoint once, reproduce a competing member identity and route/runtime socket loss, repeat disk/SQLite pressure, and complete a real browser plus node/sidecar/Root soak of at least six minutes before closure. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md) |
| `RT-POST-INCIDENT-REVIEW-002` | open | After the final post-`.803` fault-and-soak evidence is preserved, perform a separate later review of logs and counter deltas. Re-check terminal versus transient close classification, hidden resume loops, competing route owners, stale readiness/cache use, event-loop or executor starvation, persistence backlogs, skill circuit or SDK I/O-guard activity, downloads/media work, sidecar generation mismatch, and contradictions between browser, node, sidecar, and Root state. Compare the pre-failure process/I/O window with a quiet baseline, verify beacon stale responses never outlive their bounded window, and verify the supervisor-channel endpoint produces no watchdog frame in config/SQLite. This review remains open even when the first final campaign passes. | [Realtime Reliability](architecture/realtime-reliability-roadmap.md) |
