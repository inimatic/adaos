...
| `RUNTIME-DECOMPOSITION` / `WSR-001` | queued | Complete the existing extraction seams without changing public contracts: first transfer webspace task/cache/recovery/projection/materialization ownership out of the compatibility facade, then finish Bootstrap NATS/route ownership and Supervisor process/update/recovery/memory ownership. Preserve pointer-first switching, sidecar transport authority, no-op branch identity, and thin API adapters; prove each tranche with component tests and bounded `.30` stand checkpoints. | [Webspace Pointer/Projection](architecture/webspace-scenario-pointer-projection-roadmap.md#7-runtime-ownership-decomposition), [Webspace Evolution](architecture/webspace-evolution-roadmap.md#runtime-decomposition-required-by-these-gaps), [Realtime Reliability](architecture/realtime-reliability-roadmap.md#bootstrap-decomposition-constraint), [AdaOS Supervisor](architecture/adaos-supervisor.md#implementation-decomposition-boundary) |
