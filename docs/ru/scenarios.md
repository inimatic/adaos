...
Канонический manifest и единственный источник версии — `scenario.yaml`.
`scenario.json` генерируется как совместимая runtime-проекция; рассинхронизация
версий разрешается в пользу YAML, а не отдельным редактированием JSON.
