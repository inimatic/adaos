...
- `workflow.definition.v1.schema.json`, `workflow.transition.v1.schema.json`,
  ...
  and `workflow.authoring_attempt.v1.schema.json` - data-driven governed
  workflow authoring, validation, adapter trust, package binding, activation
  admission, LLM context, and provenance contracts. `workflow.definition.v1`
  references the full transition schema from the ABI folder; registry,
  admission, authoring provenance, review state, and activation pointers stay
  outside the pure definition so they cannot alter the definition digest.
