...
async def distributed_topology_phase(request: Request) -> dict[str, Any]:
    ...
    if content_length:
        try:
            if int(content_length) > MAX_TOPOLOGY_PHASE_BYTES:
                raise HTTPException(status_code=413, detail="topology_phase_request_too_large")
        except ValueError:
            raise HTTPException(
                status_code=400,
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="topology_phase_json_invalid") from exc
    if not isinstance(payload, Mapping):
        raise HTTPException(status_code=400, detail="topology_phase_body_must_be_object")
    try:
