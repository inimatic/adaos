...
def cmd_install(
    ...
    try:
        activated_slot = mgr.activate_for_space(skill_name, version=runtime.version, slot=runtime.slot, space="default", workspace_id="default")
    except Exception as exc:
...
def activate(name: str, slot: Optional[str] = typer.Option(None, "--slot"), version: Optional[str] = typer.Option(None, "--version")):
    mgr = _mgr()
    try:
        target = mgr.activate_for_space(name, version=version, slot=slot, space="default", workspace_id="default")
    except Exception as exc:
...
def rollback(name: str):
    mgr = _mgr()
    try:
        slot = mgr.rollback_for_space(name, space="default", workspace_id="default")
    except Exception as exc:
