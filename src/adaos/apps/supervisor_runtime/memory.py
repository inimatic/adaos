...
class MemoryProfilingService:
    ...
    def public_status(
        ...
        runtime = manager._memory_runtime_state_payload()
        ...
        last_session = operations.read_memory_session_summary(last_session_id) if last_session_id else None
        active_session = operations.read_memory_session_summary(active_session_id) if active_session_id else None
        ...
        return {
            ...
            "memory": {
                ...
                "profile_control_mode": str(runtime.get("profile_control_mode") or operations.implemented_profile_control_mode),
    ...
    def capture_local_incident_artifact(
        self,
        manager: Any,
        operations: MemoryProfilingOperations,
        session_id: str,
        *,
        ...
        if not token:
            return None
        summary = operations.read_memory_session_summary(token)
        ...
        now = time.time()
        operations_tail = operations.read_memory_session_operations(token, limit=200)
        telemetry_tail = manager._memory_session_telemetry_window(summary, limit=200)
        ...
        try:
            snapshot = manager._ctx.bus.backlog_snapshot() if hasattr(manager._ctx.bus, "backlog_snapshot") else {}
        ...
        artifact_dir = operations.supervisor_memory_session_artifacts_dir(token)
        ...
        manager._upsert_memory_session_summary(summary)
        return artifact_ref
