...
def development_feedback_model_rules() -> dict[str, Any]:
    ...
    return {"category": sorted(_CATEGORIES), "impact": sorted(_IMPACTS),
            ...
            "application_trace": {
                "when": "Only after an actual method or resource attempt; documentation inspection is not an execution trace.",
                "schema": "adaos.development.application_trace.v1",
                "required": ["schema", "contract_ref", "operation_id", "input_summary",
                             "expected_behavior", "observed_behavior", "validation_result"],
                "optional": ["user_response", "trace_refs"],
                "validation_result": ["passed", "failed", "unknown", "not_run"],
                "trace_refs": {"max_items": 12,
                               "item": {"type": "trace reference kind", "ref": "bounded reference"}},
                "limits": {"contract_ref": 500, "operation_id": 240, "input_summary": 1200,
                           "expected_behavior": 2000, "observed_behavior": 3000,
                           "user_response": 1000},
            },
            "clarification_questions": {
