...
def test_missing_retained_dependency_is_repaired_from_locked_package(tmp_path: Path) -> None:
    first = _built_scenario(tmp_path, version="1.0.0", marker="first")
    second = _built_scenario(tmp_path, version="1.1.0", marker="second")
    skill = _built_skill(tmp_path, version="1.0.0", marker="shared")
    store, manager = _manager(tmp_path)
    ...
    preview = manager.plan_activation(
        _plan_with_skill(second, skill),
        slot_id="primary",
    )
    assert preview["component_changes"]["repair"] == ["skill:shopping"]

    result = _activate(
        manager,
        _plan_with_skill(second, skill),
        idempotency_key="scenario-upgrade-repairs-skill",
    )

    assert skill_target.is_dir()
    assert (skill_target / "handler.py").read_text(encoding="utf-8") == (
        "MARKER = 'shared'\n"
    )
    operation = json.loads(
        manager.operation_path(result.operation_id).read_text(encoding="utf-8")
    )
    assert operation["component_plan"]["repair"] == ["skill:shopping"]
    assert {move["package"] for move in operation["moves"]} == {
        "scenario:recipes",
        "skill:shopping",
    }
