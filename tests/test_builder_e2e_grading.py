...
def test_prototype_grader_normalizes_evidence_and_separates_usage() -> None:
    ...
    def submit(messages, **kwargs):
        ...
        assert kwargs["prompt_cache_key"] == "adaos-builder-e2e-prototype-grader-v17"
        assert "BOTH parts meet their respective" in messages[0]["content"]
        assert "apply the paired-evidence rule" in messages[0]["content"]
