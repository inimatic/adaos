...
def test_copy_seed_venv_auto_uses_single_reflink_fallback_copy(monkeypatch, tmp_path: Path) -> None:
    ...
    result = mod._copy_seed_venv(source, target)
...
def test_prepared_slot_import_timeout_is_configurable_and_bounded(monkeypatch) -> None:
    ...
    monkeypatch.setenv("ADAOS_CORE_UPDATE_IMPORT_VALIDATE_TIMEOUT_SEC", "135")
    assert mod._prepared_slot_import_timeout_sec() == 135.0
