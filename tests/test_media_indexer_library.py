...
def test_media_indexer_iterator_yields_compact_playback_resources(monkeypatch, tmp_path):
    media_root = tmp_path / "library"
    media_root.mkdir()
    clip = media_root / "clip.mp4"
    song = media_root / "song.mp3"
    ...
    try:
        ...
        for playback_id, path, mime_type in (
            ("c" * 32, clip, "video/mp4"),
            ("d" * 32, song, "audio/mpeg"),
        ):
            payload = {
            ...
            connection.execute(
                "INSERT INTO items(playback_id, name, full_path, payload_json) VALUES (?, ?, ?, ?)",
                (playback_id, path.name, str(path), json.dumps(payload)),
            )
        connection.commit()
    finally:
        connection.close()
    ...
    resources = list(media_indexer_library.iter_media_indexer_resources())

    assert [item.id for item in resources] == ["c" * 32, "d" * 32]
    assert {item.source for item in resources} == {"media_indexer"}
    assert resources[0].to_public_dict()["schema"] == "adaos.media.resource.v1"
