...
def test_codex_prompt_budget_uses_one_turn_provider_overhead() -> None:
    assignment = {
        "realize_request": {
            "artifacts": {
                "execution_budget": {
    ...
    assert check["estimated_first_turn"] == {
        ...
        "billable_tokens": 15_508,
        "fresh_input_tokens": 5_508,
        "provider_input_tokens": 15_508,
        "required_primary_tokens": 6_532,
        "required_billable_tokens": 16_532,
    }
    assert check["blocked_reasons"] == [
        "primary_budget_below_estimated_first_turn",
    ]
...
def test_codex_live_budget_estimate_counts_growing_tool_context(tmp_path: Path) -> None:
    ...
    assert usage["provider_context_floor_tokens"] == 70_000
...
def test_codex_live_budget_treats_each_completed_tool_as_a_model_round(
    ...
    assert usage["tool_rounds"] == 5
    assert usage["provider_context_floor_tokens"] == 84_000
    assert usage["fresh_context_floor_tokens"] == 14_000
...
def test_codex_live_budget_enforces_primary_and_billable_limits(
    ...
    receipt = _codex_budget_exceeded_receipt(
        ...
        metric="fresh_plus_output",
        max_tokens=12_000,
        max_billable_tokens=65_000,
    )

    assert receipt is not None
    assert receipt["trigger_metric"] == "fresh_plus_output"
    assert receipt["observed_billable_tokens"] >= 70_000
...
def test_worker_resumes_blocked_candidate_across_contract_delta(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    worker = LocalSkillFactoryWorker(
    ...
    source_task_id = "task.blocked-candidate"
    source_run = worker.runs_root / source_task_id
    previous_workspace = source_run / "workspace"
    previous_file = previous_workspace / "skills" / "demo" / "webui.json"
    previous_file.parent.mkdir(parents=True)
    previous_file.write_text('{"value":"baseline"}', encoding="utf-8")
    worker._init_git_workspace(previous_workspace, "realize/source")
    previous_file.write_text('{"value":"partial-candidate"}', encoding="utf-8")
    source_contract = {
        "schema": "adaos.builder.continuation_contract.v1",
    ...
    current_contract = {
        "schema": "adaos.builder.continuation_contract.v1",
        "sdk_contract_digest": "sha256:new",
    }
    (source_run / "input").mkdir(parents=True)
    (source_run / "input" / "assignment.json").write_text(
        json.dumps(
    ...
    assert restored is not None
    assert restored["mode"] == "resume_preserved_candidate"
    assert "Contract delta required" in restored["blocking_feedback_message"]
    assert json.loads(current_file.read_text(encoding="utf-8"))["value"] == (
        "partial-candidate"
    )
