...
def test_migrate_installed_skills_runs_tests_and_rolls_back_on_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _ctx, _mgr, force=False: [
            {"skill": "weather_skill", "workspace_version": "1.2.3", "runtime_version": "", "reason": "runtime_version_behind"},
            {"skill": "service_skill", "workspace_version": "1.2.3", "runtime_version": "1.0.0", "reason": "runtime_version_behind"},
        ],
    )
...
def test_migrate_installed_skills_skips_already_deactivated_skills(monkeypatch) -> None:
    import adaos.apps.skill_runtime_migrate as mod
    ...
    class _Manager:
        ...
        def prepare_runtime(self, name: str, run_tests: bool = False):
            raise AssertionError("deactivated skills must not be prepared during migration")
    ...
    assert payload["skills"][0]["skipped"] is True
...
def test_migrate_installed_skills_marks_prepare_failures_safe_for_core_update(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "migration_candidates",
        lambda _ctx, _mgr, force=False: [
            {"skill": "broken_skill", "workspace_version": "1.2.3", "runtime_version": "1.0.0", "reason": "runtime_version_behind"}
        ],
    )
