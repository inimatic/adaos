...
#[repr(transparent)]
#[derive(Debug, Clone)]
pub(crate) struct StoreWeakRef(Weak<UnsafeCell<Store>>);

impl StoreRef {
    #[inline]
    pub(crate) fn weak_ref(&self) -> StoreWeakRef {
        StoreWeakRef(Rc::downgrade(&self.0))
    }
}

impl StoreWeakRef {
    #[inline]
    pub(crate) fn upgrade(&self) -> Option<StoreRef> {
        self.0.upgrade().map(StoreRef)
    }
}
